...
export class AuthNotifier {
  constructor({ cacheGet, cacheSet, sendTelegram, adminIds = [], loginUrl = null } = {}) {
  ...
  async notifyInvalid(reason = "refresh token invalid") {
    try {
      ...
      const keyboard = this.loginUrl
        ? { inline_keyboard: [[{ text: "重新登录 DeviantArt", url: this.loginUrl }]] }
        : undefined;
      const text =
        "⚠️ DeviantArt 登录已失效\n\n" +
        "成熟内容和部分登录后资源可能只能获得公开预览。\n\n" +
        `原因：${String(reason).slice(0, 80)}`;
